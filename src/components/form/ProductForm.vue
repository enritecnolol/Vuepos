<template>
    <div class="container">
        <form>
            <div class="row">
                <div class="col-md-6 col-sm-12 col-xs-12">
                    <div class="row">
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <div style="margin: 0px auto">
                                <input type="file" style="display: none" ref="cover" @change="onFileChanged">
                                <button type="button" @click="$refs.cover.click()" class="btn" style="margin-right: 5px"><i class="icon-plus22"></i></button>
                                <img :src="form.img" style="height: 100px; width:100px; border-radius: 50%;" alt="Logotype">
                                <button type="button" @click="deleteLogo" class="btn" style="margin-left: 5px"><i class="icon-trash"></i></button>
                            </div>
                        </div>
                    </div>
                    <div class="form-row" style="margin-top: 20px">
                        <div class="form-group col-md-12 col-sm-12 col-xs-12">
                            <label for="inputCode">Codigo</label>
                            <input
                                    style="width: 150px"
                                    type="text"
                                    class="form-control"
                                    id="inputCode"
                                    placeholder="codigo"
                                    v-model.trim="form.code"
                            >
                        </div>
                        <div class="form-group col-md-12 col-sm-12 col-xs-12">
                            <label for="inputName">Nombre</label>
                            <input
                                    style="width: 300px"
                                    type="text"
                                    class="form-control"
                                    id="inputName"
                                    placeholder="Nombre..."
                                    v-model.trim="form.name"
                            >
                        </div>
                        <div class="form-group col-md-12 col-sm-12 col-xs-12">
                            <label for="inputPrice">Precio</label>
                            <input
                                    style="width: 150px"
                                    type="text"
                                    class="form-control"
                                    id="inputPrice"
                                    placeholder="precio"
                                    v-model.trim="form.price"
                            >
                        </div>
                        <div class="form-group col-md-12 col-sm-12 col-xs-12">
                            <label for="inputCategory">Categoria</label>
                            <select class="form-control" id="inputCategory" v-model="form.category" style="width: 250px">
                                <option value="opt1" selected>Default select box</option>
                                <option value="opt2" >Option 2</option>
                                <option value="opt3" >Option 3</option>
                                <option value="opt4" >Option 4</option>
                            </select>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row" style="margin-top: 20px">
                <div class="col-md-12 col-sm-12">
                    <div class="text-center">
                        <button type="button" class="btn btn-lg btn-warning" style="margin-right: 20px;"> <i class="icon-reload-alt"></i> Reset</button>
                        <button type="button" class="btn btn-lg btn-primary"><i class="icon-upload4"></i> Submit</button>
                    </div>
                </div>
            </div>
        </form>
    </div>
</template>

<script>
    const _validFileExtensions = ["jpg", "jpeg", "png"];
    const re = /(?:\.([^.]+))?$/;
    export default {
        data(){
            return{
                form:{
                    name:'',
                    code:"",
                    price:"",
                    category:"opt1",
                    img:"data:image/png;base64,"
                },
                selectedFile:""
            }
        },
        methods:{
            onFileChanged(event) {
                this.selectedFile = event.target.files[0];
                if (this.selectedFile) {
                    const ext = re.exec(this.selectedFile.name)[1];
                    if (_validFileExtensions.indexOf(ext) !== -1) {
                        var reader = new FileReader();
                        reader.readAsDataURL(this.selectedFile);
                        reader.onload = () => {
                            this.form.img = reader.result;
                            return;
                        };
                    }
                    this.deleteLogo();
                }
            },
            deleteLogo() {
                this.form.img = "data:image/png;base64,";
            },
        }

    }
</script>

<style scoped>

</style>